...
def training_loss(dataloader, optimizer, model, mystudy): #todo : new name
    ...
    for batch_data in dataloader:  # the input image is also the label
        ...
        running_acc += result_accuracy(pred, batch_data)
...
def testing_loss(testloader, model, mystudy):
    ...
    with torch.no_grad():  # weights kept constant - no .backward() needed
        for batch_data in testloader:
            ...
            running_acc += result_accuracy(output, batch_data)
