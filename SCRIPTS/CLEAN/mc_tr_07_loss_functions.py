...
def training_loss(dataloader, optimizer, model, myparams, threshold): #todo : new name
    ...
    for batch_data in dataloader:  # the input image is also the label
        # Train model
        batch_data = batch_data.to(myparams.device) # b_d : batchsize*(7699/groupsize) ratings

        # a = batch_data.cpu().detach().numpy()
    ...
    tr_loss = running_loss / len(dataloader.dataset) #TODO : check replaced denom to len(dataloader)
    tr_acc = running_acc/len(dataloader) #TODO : check replaced denom to len(dataloader.dataset)
...
def testing_loss(testloader, model, myparams, threshold ):
    ...
    te_loss = running_autorec_loss / len(testloader.dataset) #TODO : check replaced denom to len(testloader)
    rmse_loss = running_rmse_loss / len(testloader.dataset) #TODO : check replaced denom to len(testloader)
    te_acc = running_acc/len(testloader) #TODO : check replaced denom to len(testloader.dataset)
